...
const continuationDraftValidator = v.object({
...
export const complete = mutation({
  args: {
    ...
    continuations: v.array(continuationDraftValidator),
  ...
  handler: async (ctx, args) => {
    ...
    const itemId = safeText(args.id, "Item ID", 240);
    const summary = safeOptionalText(args.summary, "Summary", 10_000);
    if (args.continuations.length > 20) {
      throw new Error("A completion may contain at most 20 continuations");
    }
    const drafts = args.continuations.map(normalizeDraft);
    const request = {
      id: itemId,
      actor: publicActor(args.actor),
      summary: summary ?? null,
      continuations: drafts,
    };
    ...
    if (idempotencyKey) {
      const replay = await ctx.db
        ...
        .unique();
      if (replay) {
        if (replay.itemExternalId !== itemId) {
          throw new Error(
    ...
    const now = Date.now();
    let item = await getItemByExternalId(ctx, workspace._id, itemId);
    ...
    for (const draft of drafts) {
      ...
      await ctx.db.patch(continuationId, {
        sourceEventExternalId: event.id,
      });
    ...
    if (!updated) throw new Error("Completed item disappeared");
    const result = {
      item: await publicItem(ctx, updated),
      continuations,
    };
    if (idempotencyKey) {
...
function normalizeDraft(draft: ContinuationDraft) {
  return {
    ...
    approvalMode: draft.approvalMode ?? "human",
    deliveryMode: draft.deliveryMode ?? "human_inbox",
    expiresAt: normalizeTimestamp(draft.expiresAt, "Continuation expiry"),
  };
}
...
function requireSameRequest(existing: unknown, requested: unknown) {
