...
Names come from a vibe. The default `ops` pool is short aviation and operations callsigns (Talon, Viper, Ironhawk); `--vibe whimsical` draws gentle everyday words (Teakettle, Lantern, Quillmoor). `--vibe lame` is proudly corny (Meatloaf, Kerfuffle, Spudnugget). A team can pin one with `{"vibe": "whimsical"}` in a `.callsign.json` at its repository root, a person in `$XDG_CONFIG_HOME/callsign/config.json`, or either with `CALLSIGN_VIBE`. The vibe chooses words only; sigil and collision key never depend on it.
