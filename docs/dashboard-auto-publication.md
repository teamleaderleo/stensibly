...
5. fails closed on discovery errors, malformed responses, repeated/invalid cursors, contradictory target records, conflicting source projects, or invalid project identities;
...
`test/link-vercel-project-domain.test.ts` executes the domain script against a local fake `curl`. It covers an existing binding, complete paginated move discovery, add after proven absence, discovery failure with hostile provider text, invalid source identity, mutation failure, exact request bodies, and the final target-project postcondition. No real Vercel request or credential is used by the test.
