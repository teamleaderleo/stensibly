...
Dashboard production changes are published through a two-hour release window.
...
5. fails closed on discovery errors, malformed responses or records, repeated or invalid cursors, contradictory target records, conflicting source projects, or invalid project identities;
6. moves `www.stensibly.com` from the one discovered project in the same Vercel team, or adds it to project `stensibly` only after complete discovery proves no source project;
7. requires Vercel to confirm the domain's exact target project identity after mutation;
8. pulls project settings and builds the complete configured project from the repository root;
