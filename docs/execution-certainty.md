...
Colon-free identifiers use the neutral bounded identifier grammar. Colon-bearing
identities are limited to these exact forms:

```text
github:<resource-kind>:<identity>
provider-result:github:<resource-kind>:<identity>
```

The closed resource kinds are `issue`, `pull-request`, `workflow-run`, `job`,
`result`, `artifact`, and `commit`. Identity segments contain no slash or colon.
Direct, embedded, opaque, prefixed, and namespace-wrapped URI schemes such as
`https:`, `file:`, `mailto:`, `urn:`, and `tel:` therefore fail before a receipt
can retain or fingerprint them.
...
record with its originating receipt, the parser rechecks whether reconciliation
was required, the receipt fingerprint, operation identity, request and authority
fingerprints, run generation, and chronology. A caller must use a separately
reviewed retry or recovery policy; this module never grants one.

## Integrity and recovery

Receipts and reconciliation records are deeply frozen. Their derived
projections and receipt and reconciliation fingerprints are recomputed during
parsing. Request, authority, and evidence fingerprints are validated as
SHA-256 digests and bound into the record. Callers cannot alter certainty,
resolution, or replay status independently from the admitted evidence.
