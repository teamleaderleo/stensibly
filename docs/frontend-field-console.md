...
The ambiguous publication action says **Reconcile before retry**. Activating it only announces the exact safe next action. It performs no retry, write, network request, or simulated settlement.
...
The route temporarily duplicates the shared fictional identities so it remains independently previewable in the catalogue's opaque `sandbox="allow-scripts"` frame. After the shared classic fixture bridge is merged and stable, a separate reviewed change can replace the duplicate source while preserving exact task identities and route behavior.
