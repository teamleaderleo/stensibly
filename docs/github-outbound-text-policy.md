...
Version one remains authority-free. Every reference outside the destination and
configured controlled owner/repository set is rejected with
`external_reference_requires_authority`.

A caller may supply the legacy external-contact authority-shaped input only as
bounded compatibility data. The evaluator exact-admits its fields, verifies its
canonical repository set, checksum, and generation, then grants it zero effect.
A caller who knows the public checksum algorithm cannot authorize an external
repository.

Every emitted receipt therefore carries:

```text
externalContactAuthorityFingerprint: null
externalContactAuthorityGeneration: null
referenceCounts.authorized: 0
providerDispatchAuthorized: false
```

Receipt parsing rejects any claimed external authority identity or authorized
reference count, even when a caller recomputes the outer receipt fingerprint.
A later slice may consume a server-owned authority projection whose subject,
action, destination scope, generation, validity, approval, and fingerprint are
reconstructed against trusted state.
...
Controlled repository sets and compatibility authority-claim repository sets
are canonical, unique, sorted, and admitted through the same dense-array
boundary.
...
- external authority fingerprint and generation remain null;
- authorized reference count remains zero;
