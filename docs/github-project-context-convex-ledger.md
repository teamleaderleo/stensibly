...
Observation chronology is checked before instruction rebound. Older evidence is retained as `stale` history and cannot demote the current row. A single Convex transaction patches the prior current row and appends the new generation, so concurrent acceptance leaves one current generation.
