...
8. Run the OAuth discovery and challenge verifier against the official and fallback endpoints.
...
The OAuth verifier is unauthenticated and read-only. It checks the healthy hosted surface, both metadata documents, the required-token MCP challenge, and the invalid-token challenge. It does not register a client, start a login, exchange a token, or mutate Convex state.

The verifier rejects redirects so the fallback command cannot silently certify the canonical endpoint. Response bodies and `WWW-Authenticate` headers are read under the same timeout, physically bounded, validated, and never copied into diagnostics. Only a bounded validated request ID may be retained from a failed response.
