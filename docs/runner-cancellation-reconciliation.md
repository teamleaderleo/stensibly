...
The reconciliation result retains that same command fingerprint, so downstream release evidence remains attributable to the complete cancellation command. A caller cannot substitute a different valid SHA-256 value and recompute only the outer result fingerprint: the v2 receipt must carry that exact digest.
