(() => {
  ...
  function parseFrontendLabFixture(value) {
    const record = exactRecord(value, fixtureKeys, "Frontend labs fixture");
    const parsed = {
      project: parseEntity(record.project, ["id", "name", "summary"], "project"),
      decision: parseStateEntity(record.decision, ["detail", "id", "state", "title"], "decision"),
      workers: parseList(record.workers, 1, 8, (entry, index) => parseStateEntity(entry, ["detail", "id", "label", "state"], `worker ${index + 1}`)),
      readyWork: parseList(record.readyWork, 1, 12, (entry, index) => {
        const ready = exactRecord(entry, ["id", "rank", "reason", "state", "title"], `ready work ${index + 1}`);
        if (!Number.isSafeInteger(ready.rank) || ready.rank < 1 || ready.rank > 99) throw new TypeError("Ready work rank must be 1-99");
        return Object.freeze({ id: slug(ready.id, "Ready work id"), rank: ready.rank, reason: text(ready.reason, 240, "Ready work reason"), state: state(ready.state), title: text(ready.title, 100, "Ready work title") });
      }),
      operations: parseList(record.operations, 1, 12, (entry, index) => parseStateEntity(entry, ["action", "detail", "id", "state", "title"], `operation ${index + 1}`)),
      connections: parseList(record.connections, 1, 8, (entry, index) => parseStateEntity(entry, ["detail", "id", "label", "state"], `connection ${index + 1}`)),
      references: parseList(record.references, 1, 12, (entry, index) => parseEntity(entry, ["kind", "label", "value"], `reference ${index + 1}`)),
    };
    assertUniqueIds([
      parsed.decision.id,
      ...parsed.workers.map((entry) => entry.id),
      ...parsed.readyWork.map((entry) => entry.id),
      ...parsed.operations.map((entry) => entry.id),
      ...parsed.connections.map((entry) => entry.id),
    ], "Frontend labs record ids");
    return deepFreeze(parsed);
  }

  function parseFrontendLabTasks(value) {
    const tasks = parseList(value, 1, 20, (entry, index) => {
      const task = exactRecord(entry, taskKeys, `task ${index + 1}`);
      return Object.freeze({ id: slug(task.id, "Task id"), prompt: text(task.prompt, 180, "Task prompt"), start: slug(task.start, "Task start"), success: text(task.success, 120, "Task success") });
    });
    assertUniqueIds(tasks.map((task) => task.id), "Frontend labs task ids");
    return tasks;
  }
  ...
  function createFrontendLabReport(taskIds = frontendLabTasks.map((task) => task.id)) {
    ...
    const admittedIds = parseList(taskIds, 0, frontendLabTasks.length, (id) => slug(id, "Report task id"));
    assertUniqueIds(admittedIds, "Report task ids");
    const tasks = admittedIds.map((normalized) => {
  ...
  function parseStateEntity(value, keys, label) {
    ...
    const result = {};
    for (const key of keys) result[key] = key === "id" ? slug(record[key], `${label} id`) : key === "state" ? state(record[key]) : text(record[key], 240, `${label} ${key}`);
    return Object.freeze(result);
  }
  function parseEntity(value, keys, label) {
    const record = exactRecord(value, keys, label);
    const result = {};
    for (const key of keys) result[key] = key === "id" ? slug(record[key], `${label} id`) : text(record[key], 240, `${label} ${key}`);
    return Object.freeze(result);
  }
  function parseList(value, minimum, maximum, parser) {
    if (!Array.isArray(value) || value.length < minimum || value.length > maximum) throw new TypeError(`Expected ${minimum}-${maximum} entries`);
    if (Object.getPrototypeOf(value) !== Array.prototype) throw new TypeError("Lists must use the default array prototype");
    if (Object.getOwnPropertySymbols(value).length > 0) throw new TypeError("Lists must not contain symbol fields");
    const descriptors = Object.getOwnPropertyDescriptors(value);
    for (const key of Object.keys(descriptors)) {
      if (key === "length") continue;
      if (!/^(?:0|[1-9][0-9]*)$/.test(key) || Number(key) >= value.length) throw new TypeError(`List contains unsupported field ${key}`);
    }
    const result = [];
    for (let index = 0; index < value.length; index += 1) {
      const descriptor = descriptors[String(index)];
      if (!descriptor) throw new TypeError("Lists must be dense");
      if (!descriptor.enumerable || !("value" in descriptor)) throw new TypeError(`List entry ${index + 1} must be an enumerable data property`);
  ...
  function exactRecord(value, keys, label) {
    ...
    const descriptors = Object.getOwnPropertyDescriptors(value);
    if (Object.keys(descriptors).sort().join(",") !== [...keys].sort().join(",")) throw new TypeError(`${label} must use exact fields`);
    const result = {};
    for (const key of keys) {
      const descriptor = descriptors[key];
      if (!descriptor || !descriptor.enumerable || !("value" in descriptor)) throw new TypeError(`${label} field ${key} must be an enumerable data property`);
      result[key] = descriptor.value;
    }
    return result;
  }
  function assertUniqueIds(ids, label) { if (new Set(ids).size !== ids.length) throw new TypeError(`${label} must be unique`); }
  ...
  function isCompatibleApi(value) {
    ...
    const descriptors = Object.getOwnPropertyDescriptors(value);
    if (Object.keys(descriptors).sort().join(",") !== apiKeys.join(",")) return false;
    ...
    return Boolean(fixture)
      ...
      && Array.isArray(tasks)
      && Object.isFrozen(tasks)
      && [descriptors.parseFrontendLabFixture.value, descriptors.parseFrontendLabTasks.value, descriptors.createFrontendLabReport.value]
