...
function canonicalPriorHolder(
  ...
  const callsign = historicalCallsign(input.callsign, "Prior callsign");
...
function historicalCallsign(
  ...
  if ([...value].length > limits.callsign) {
    throw new RangeError(`${label} must be at most ${limits.callsign} characters`);
  }
  return {
    display: value,
    collisionKey: callsignCollisionKey(value),
  };
}
