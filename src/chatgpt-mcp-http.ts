...
import { createChatGptMcpServer } from "./chatgpt-mcp.js";
...
export interface ChatGptMcpHttpOptions {
...
export async function handleChatGptMcpHttpRequest(
  ...
  const server = createChatGptMcpServer(options.ledger);
...
async function resolveAccessRule(
  ...
  if (
    ...
    toolName === "show_continuation_card"
  ) {
