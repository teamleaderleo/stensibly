  ...
  implements GitHubInstallationTokenProvider
{
  ...
  async getInstallationToken(
    ...
    try {
      const providerCall = Promise.resolve().then(() => this.#fetch(
    ...
    let payload: unknown;
    try {
      const status = admitResponseStatus(response);
      if (!status.ok) {
        discardResponseBody(response);
        throw githubHttpError(status.status, "mint installation token");
      }
...
function admitResponseStatus(response: Response): {
  ok: boolean;
  status: number;
} {
  let ok: boolean;
...
async function readBoundedJson(
  ...
  try {
    ...
    if (declaredLength !== null && typeof declaredLength !== "string") {
