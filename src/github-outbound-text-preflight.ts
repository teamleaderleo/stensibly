...
function detectReferences(text: string): readonly DetectedReference[] {
  ...
  const directItemReferencePattern =
    /https?:\/\/(?:www\.)?github\.com\/([A-Za-z0-9][A-Za-z0-9-]{0,38})\/([A-Za-z0-9][A-Za-z0-9_.-]{0,99})\/(issues|pull|discussions)\/([0-9]+)(?![\p{L}\p{N}\p{M}_-])/giu;
  const directCommitReferencePattern =
    /https?:\/\/(?:www\.)?github\.com\/([A-Za-z0-9][A-Za-z0-9-]{0,38})\/([A-Za-z0-9][A-Za-z0-9_.-]{0,99})\/commit\/([0-9a-f]{7,})(?![\p{L}\p{N}\p{M}_-])/giu;
  ...
  const commitShorthandPattern =
    /(?:^|[^\p{L}\p{N}\p{M}._~/?#@!$&*+=%-])([A-Za-z0-9][A-Za-z0-9-]{0,38})\/([A-Za-z0-9][A-Za-z0-9_.-]{0,99})@([0-9a-f]{7,})(?![\p{L}\p{N}\p{M}_~\/?#@!$&*+=%-])/giu;
