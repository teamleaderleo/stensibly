...
const instructionSetCredentialDiagnostic =
...
function admitBinding(value: unknown): HostedGitHubIssueContextBindingInputV1 {
  ...
  assertInstructionSetRetainedCredentialSafety(instructionSet);
...
function assertInstructionSetRetainedCredentialSafety(
  value: AcceptedRepositoryInstructionSet,
): void {
  if (
    value.sources.some((source) =>
      containsRealisticRetainedCredential(source.path)
      || containsRealisticRetainedCredential(source.revision)
    )
  ) {
    throw new RangeError(instructionSetCredentialDiagnostic);
  }
}
