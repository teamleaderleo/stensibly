...
const retainedCredentialDiagnostic =
...
export function resolveGitHubProviderInstructionObservationV1(
  ...
  assertRetainedCredentialSafety(base);
...
function assertRetainedCredentialSafety(value: BaseResolution): void {
  const retained = [
    value.workspace,
    value.project,
    ...
    value.attachmentId,
    value.observedBy,
    ...(value.instructionSet?.sources.flatMap((source) => [
      source.path,
      source.revision,
    ]) ?? []),
  ];
  if (
    retained.some((candidate) =>
      candidate !== null && containsRealisticRetainedCredential(candidate)
    )
  ) {
    throw new RangeError(retainedCredentialDiagnostic);
  }
}
