...
export class GitHubRestActionsRunAdapter extends GitHubRestDelegatedReadAdapter {
  ...
  #admit(envelope: Record<string, unknown>): AdmittedCall {
    ...
    const repositoryFullName = repository(envelope.repositoryFullName);
    if (envelope.tool === "fetch_commit_workflow_runs") {
      ...
      return Object.freeze({
        tool: envelope.tool,
        repositoryFullName,
        commitSha: commitSha(args.commit_sha),
        runId: null,
      });
    }
    const args = exactRecord(
    ...
    return Object.freeze({
      ...
      repositoryFullName,
      commitSha: null,
      runId: integer(args.run_id, "workflow run ID", 1, Number.MAX_SAFE_INTEGER),
    });
  }
...
function parseRun(value: unknown, admitted: AdmittedCall): Readonly<Record<string, unknown>> {
  ...
  if (commitSha(run.head_sha) !== admitted.commitSha) {
...
function parseJob(
  ...
  verifyJobUrl(job.url, base, admitted.repositoryFullName, runId, job.id);
  ...
  return Object.freeze({
    ...
    headSha: commitSha(job.head_sha),
...
function verifyUrl(url: string, base: string, admitted: AdmittedCall): void {
  ...
  const expectedCount = admitted.tool === "fetch_commit_workflow_runs" ? 3 : 3;
  if (
    ...
    || parsed.searchParams.size !== expectedCount
    || (admitted.tool === "fetch_commit_workflow_runs" && parsed.searchParams.get("head_sha") !== admitted.commitSha)
    || (admitted.tool === "fetch_workflow_run_jobs" && parsed.searchParams.get("filter") !== "all")
  ) throw invalid("GitHub delegated pagination URL escaped the accepted request");
}

function verifyJobUrl(value: unknown, base: string, repositoryFullName: string, runId: number, jobId: unknown): void {
...
function jobsUrl(base: string, repo: string, runId: number, page: number): string {
  const [owner, name] = repo.split("/");
  return `${base}/repos/${encodeURIComponent(owner!)}/${encodeURIComponent(name!)}/actions/runs/${runId}/jobs?filter=all&per_page=${jobsPageSize}&page=${page}`;
}
