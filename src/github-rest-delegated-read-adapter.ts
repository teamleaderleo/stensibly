...
const credentialShapedPublicIdentityPattern = /(?:^|[\s:./=,;'"()\[\]{}@#-])(?:Bearer\s+|gh[pousr]_|github_pat_|sk-[A-Za-z0-9]|stn\.tok_|xox[baprs]-|env:\/\/|secret:\/\/|eyJ[A-Za-z0-9_-]+\.eyJ[A-Za-z0-9_-]+\.)/iu;
...
const minimumResponseChunks = 4_096;
...
async function readBoundedJson(
  ...
  const chunkBound = responseChunkBound(maximumBytes);
  let total = 0;
  ...
  try {
    while (true) {
      ...
      if (read.done) break;
      deliveredChunks += 1;
      if (deliveredChunks > chunkBound) {
        void cancelReader(reader);
        throw rejected(
          "github_delegated_provider_result_too_large",
          `GitHub delegated provider response exceeds ${chunkBound} chunks`,
        );
      }
...
function responseChunkBound(maximumBytes: number): number {
  return Math.max(minimumResponseChunks, maximumBytes + 1);
}
...
function detachResponseChunk(value: unknown, maximumBytes: number): Uint8Array {
  ...
  try {
    typedArrayTag = typedArrayTagGetter?.call(value);
  } catch {
