...
export type InMemoryMailProviderMode = "normal" | "ambiguous_after_send" | "definite_failure";
...
export class InMemoryMailProvider implements MailProvider {
  ...
  constructor(provider = "fake") {
    if (!/^[a-z][a-z0-9_-]{0,31}$/u.test(provider)) throw new TypeError("In-memory mail provider name is invalid");
    this.provider = provider;
  }

  setMode(mode: InMemoryMailProviderMode): void { this.#mode = mode; }
  ...
  messagesForThread(providerThreadId: string): readonly MailProviderMessage[] {
    return Object.freeze((this.#threadMessages.get(providerThreadId) ?? []).map((stored) => stored.message));
  }
  ...
  async replyThread(
    ...
    if (!known || known.length === 0) throw new MailProviderDefiniteFailure("mail_provider_thread_missing");
  ...
  async getDeliveryProjection(
    ...
    const binding = freezeMailboxBinding(bindingInput);
    this.#assertProvider(binding);
    if (input.rfcMessageId === null) return { status: "missing", coverage: "unknown" };
    ...
    if (input.expectedProviderThreadId !== null && candidate.result.providerThreadId !== input.expectedProviderThreadId) {
  ...
  #send(
    ...
    if (this.#mode === "definite_failure") throw new MailProviderDefiniteFailure("mail_provider_rejected");
    this.#counter += 1;
    ...
    if (this.#mode === "ambiguous_after_send") throw new MailProviderAmbiguousFailure("mail_provider_response_lost");
    return result;
  }

  #message(input: MailProviderMessage): MailProviderMessage & { rfcMessageId: string } {
    const message = freezeMailProviderMessage(input);
    if (message.rfcMessageId === null) throw new MailProviderDefiniteFailure("mail_provider_requires_caller_assigned_rfc_message_id");
    return message as MailProviderMessage & { rfcMessageId: string };
  }

  #assertProvider(binding: MailboxBinding): void {
    if (binding.provider !== this.provider) throw new MailProviderDefiniteFailure("mail_provider_binding_mismatch");
  }
}
