...
export class MailOutboundService {
  ...
  async publish(command: PublishMailThreadCommand): Promise<MailPublishResult> {
    ...
    const projection = projectionFromSend(
      thread,
      binding,
      envelope,
      message,
  ...
  async reconcile(input: {
    ...
    if (!thread || thread.threadId !== effect.threadId || effect.accountBinding !== binding.accountBinding) {
    ...
    if (lookup.status === "found") {
      ...
      const nextProjection = projectionFromSend(
        thread,
        binding,
        envelope,
        message,
  ...
  async #ensureThread(command: PublishMailThreadCommand): Promise<MailThreadRecord> {
    ...
    for (let attempt = 0; attempt < 8; attempt += 1) {
      ...
      if (reservation.outcome === "source_conflict") {
        throw new MailThreadSourceConflictError(reservation.thread);
      }
    }
...
function createEffect(
  ...
  const outboundEffectId = `mailfx_${hex.slice(0, 40)}`;
