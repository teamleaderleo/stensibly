...
<form id="consent-approve" class="fallback-form" method="post" action="/oauth/consent">${consentFields("approve")}<input class="consent-control approve native-fallback" type="submit" value="Connect ${clientName}"></form><form id="consent-deny" class="fallback-form" method="post" action="/oauth/consent">${consentFields("deny")}<input class="consent-control native-fallback" type="submit" value="Not now"></form><noscript><style>.js-actions{display:none}.fallback-form{display:block;margin-top:10px}</style><p class="fine-print">JavaScript is off. Use a button below to continue.</p></noscript></section></main></body></html>`;
}
