...
function compileThread(
  ...
  const latestContradiction = contradictions.at(-1) ?? null;
  const resolverSettlesContradictions = resolver !== null
    && (
      latestContradiction === null
      || compareObservations(resolver, latestContradiction) > 0
    );
  const state: OrchestratorAttentionThreadState =
    contradictions.length > 0 && !resolverSettlesContradictions
      ? "contradictory"
...
function isExactProviderResolver(
  ...
  return observation.sourceClass === "provider_observation"
    && observation.activityState === "succeeded"
...
function evidenceFingerprint(ids: readonly string[]): string {
