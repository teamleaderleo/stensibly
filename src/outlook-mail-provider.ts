...
export class OutlookMailProvider implements MailProvider {
  ...
  async createThread(
    ...
    return this.#dispatchCreate(binding, message);
  }
  ...
  async getDeliveryProjection(
    ...
    try {
      rawCandidates = await this.#client.findMessagesByOutboundEffectId({
        outboundEffectId,
      });
    } catch {
    ...
    for (const candidate of rawCandidates) {
      try {
        ...
        return {
          status: "ambiguous",
          candidateCount: Math.max(2, rawCandidates.length),
        };
      }
    }
    ...
    if (
      ...
      && candidate.conversationId !== expectedProviderThreadId
    ) {
