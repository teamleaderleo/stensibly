...
async function initCommand(args: string[]): Promise<void> {
  ...
  console.log(JSON.stringify({
    ...
    next: `Review ${path}, then run bun run attach compile --out .stensibly/project-attachment.json`,
  }, null, 2));
}
...
STENSIBLY.md is authoring input. The compile command produces a canonical, hashed attachment snapshot intended for explicit API import. Neither file is a credential, lease, approval, or live authority grant.`;
}
