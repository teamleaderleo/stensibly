...
const LIST_KEYS = new Set(["repositories", "runner_profiles", "autonomous_actions", "approval_required", "checks"]);
const ACTIONS = new Set<string>(PROJECT_ATTACHMENT_ACTIONS);
const BODY_HEADINGS = ["Goal", "Boundaries", "Evidence and handoff expectations", "Escalation"] as const;
...
export type ComparableProjectAttachmentContract = Omit<CanonicalProjectAttachmentContract, "version"> & { version: number };
...
export function parseProjectAttachmentContract(
  ...
  return { ok: true, value: { contract, source, digestInput: projectAttachmentDigestInput(contract) } };
}
...
export function assertProjectAttachmentContract(
  ...
  throw new Error(`Invalid ${PROJECT_ATTACHMENT_FILENAME}: ${result.errors.map((item) => `${item.path}: ${item.message}`).join("; ")}`);
}

export function projectAttachmentDigestInput(contract: ComparableProjectAttachmentContract): string {
  return JSON.stringify({
    version: contract.version,
    project: contract.project,
    repositories: [...contract.repositories],
    runnerProfiles: [...contract.runnerProfiles],
    concurrency: { project: contract.concurrency.project, global: contract.concurrency.global },
...
function splitFrontMatter(content: string, errors: Errors): { frontMatter: string[]; body: string } | null {
...
function parseFrontMatter(lines: string[], errors: Errors): RawFrontMatter | null {
  ...
  while (index < lines.length) {
    ...
    if (seen.has(key)) error(errors, "duplicate_key", `front_matter.${key}`, "Duplicate key after normalisation");
    seen.add(key);
    if (!TOP_KEYS.includes(key as typeof TOP_KEYS[number])) error(errors, "unknown_key", `front_matter.${key}`, "Unknown top-level policy key");
    if (rawKey !== key) error(errors, "noncanonical_key", `front_matter.${key}`, `Use the canonical key ${key}`);
    const inline = (match[2] ?? "").trim();

    if (key === "concurrency") {
      if (inline) error(errors, "invalid_value", "front_matter.concurrency", "Concurrency must be a two-key block");
    ...
    if (LIST_KEYS.has(key)) {
      if (inline) error(errors, "invalid_value", `front_matter.${key}`, "Lists must use one indented - item per line");
      const parsed = parseList(lines, index + 1, key, errors);
      assignList(result, key, parsed.values);
      index = parsed.next;
      continue;
    }
    if (!inline) error(errors, "missing_field", `front_matter.${key}`, "A scalar value is required");
    else if (key === "version" || key === "project") result[key] = scalar(inline, `front_matter.${key}`, errors);
    index += 1;
  }
  return errors.some((item) => item.code === "malformed_front_matter") ? null : result;
}

function parseConcurrency(lines: string[], start: number, errors: Errors): { value: Record<string, string>; next: number } {
  ...
  while (index < lines.length) {
    ...
    const rawKey = match[1] ?? "";
    const key = rawKey.toLowerCase().replace(/-/g, "_");
    if (seen.has(key)) error(errors, "duplicate_key", `front_matter.concurrency.${key}`, "Duplicate concurrency key");
    seen.add(key);
    if (rawKey !== key) error(errors, "noncanonical_key", `front_matter.concurrency.${key}`, `Use the canonical key ${key}`);
    if (key !== "project" && key !== "global") error(errors, "unknown_key", `front_matter.concurrency.${key}`, "Unknown concurrency key");
    else value[key] = scalar(match[2] ?? "", `front_matter.concurrency.${key}`, errors);
    index += 1;
  }
  return { value, next: index };
}

function parseList(lines: string[], start: number, key: string, errors: Errors): { values: string[]; next: number } {
  const values: string[] = [];
  let index = start;
  while (index < lines.length) {
    const line = lines[index] ?? "";
...
function parseBody(input: string, errors: Errors): ProjectAttachmentBody | null {
  ...
  for (const heading of BODY_HEADINGS) {
    ...
    if (count === 0) error(errors, "missing_section", `body.${bodyKey(heading)}`, `Missing ${heading} section`);
    if (count > 1) error(errors, "duplicate_section", `body.${bodyKey(heading)}`, `More than one ${heading} section`);
  }
  const recognized = found.filter((entry) => BODY_HEADINGS.includes(entry.heading as typeof BODY_HEADINGS[number]));
  if (recognized.length === BODY_HEADINGS.length && recognized.some((entry, index) => entry.heading !== BODY_HEADINGS[index])) {
    error(errors, "section_order", "body", `Sections must appear in this order: ${BODY_HEADINGS.join(", ")}`);
  }
  if (errors.some((item) => ["missing_section", "duplicate_section", "unknown_section", "section_order"].includes(item.code))) return null;
  ...
  for (const [position, entry] of recognized.entries()) {
    ...
    const text = lines.slice(entry.index + 1, next).map((line) => line.replace(/[ \t]+$/g, "")).join("\n").trim();
    const key = bodyKey(entry.heading as typeof BODY_HEADINGS[number]);
    if (!text) error(errors, "invalid_value", `body.${key}`, `${entry.heading} must not be empty`);
    if (text.length > 10_000) error(errors, "section_too_large", `body.${key}`, `${entry.heading} exceeds 10,000 characters`);
    sections[key] = text;
  }
...
function canonicalize(raw: RawFrontMatter, body: ProjectAttachmentBody, errors: Errors): CanonicalProjectAttachmentContract | null {
  for (const key of TOP_KEYS) if (raw[key] === undefined) error(errors, "missing_field", `front_matter.${key}`, `Missing required field ${key}`);
  if (errors.length) return null;
  const version = integer(raw.version ?? "", 1, 999, "front_matter.version", errors);
  if (version !== null && version !== PROJECT_ATTACHMENT_VERSION) error(errors, "unsupported_version", "front_matter.version", "Only version 1 is supported");
  const project = simple(raw.project ?? "", 80)?.toLowerCase();
  if (!project || !/^[a-z0-9][a-z0-9-]*$/.test(project)) error(errors, "invalid_value", "front_matter.project", "Project must be a lowercase slug using letters, digits, and hyphens");

  const repositories = list(raw.repositories ?? [], repository, "front_matter.repositories", 1, MAX_LIST, errors);
  const runnerProfiles = list(raw.runner_profiles ?? [], namedIdentifier, "front_matter.runner_profiles", 1, MAX_LIST, errors);
  const autonomousActions = actionList(raw.autonomous_actions ?? [], "front_matter.autonomous_actions", errors);
  const approvalRequired = actionList(raw.approval_required ?? [], "front_matter.approval_required", errors);
  const checks = list(raw.checks ?? [], command, "front_matter.checks", 1, 24, errors, false);
  const projectConcurrency = integer(raw.concurrency?.project ?? "", 1, 16, "front_matter.concurrency.project", errors);
  const globalConcurrency = integer(raw.concurrency?.global ?? "", 1, 64, "front_matter.concurrency.global", errors);
  ...
  for (const action of REQUIRED_APPROVAL_ACTIONS) {
    if (!approvalSet.has(action)) error(errors, "invalid_value", "front_matter.approval_required", `Version 1 requires approval for ${action}`);
  }
  const overlap = autonomousActions.filter((action) => approvalSet.has(action));
  if (overlap.length) error(errors, "invalid_value", "front_matter.autonomous_actions", `Actions cannot be both autonomous and approval-required: ${overlap.join(", ")}`);
  if (errors.length || version === null || !project || projectConcurrency === null || globalConcurrency === null) return null;
  return {
...
function normalizeSource(input: ProjectAttachmentSourceMetadata, errors: Errors): ProjectAttachmentSourceMetadata | null {
  const path = simple(input.path, 4096);
  ...
  if (!path || secretShaped(path)) error(errors, "invalid_value", "source.path", "Source path must be bounded credential-free text");
  if (input.repository !== undefined && !repo) error(errors, "invalid_value", "source.repository", "Source repository must use owner/repository");
  if (input.revision !== undefined && (!revision || secretShaped(revision))) error(errors, "invalid_value", "source.revision", "Source revision must be bounded credential-free text");
  if (errors.length || !path) return null;
  return { path, ...(repo ? { repository: repo } : {}), ...(revision ? { revision } : {}) };
}

function actionList(values: string[], path: string, errors: Errors): ProjectAttachmentAction[] {
  return list(values, (value) => {
    const normalized = simple(value, 64)?.toLowerCase().replace(/-/g, "_");
    return normalized && ACTIONS.has(normalized) ? normalized as ProjectAttachmentAction : null;
...
function repository(value: string): string | null {
  ...
  return parts[0] && parts[1] && segment.test(parts[0]) && segment.test(parts[1]) ? `${parts[0]}/${parts[1]}` : null;
}
...
function command(value: string): string | null {
  ...
  if (!normalized || secretShaped(normalized) || /[;&|`<>]|\$\(|\$\{|\\\s*$/.test(normalized)) return null;
  if (/^[A-Za-z_][A-Za-z0-9_]*=/.test(normalized) || !/^[A-Za-z0-9_./:@%+=,* -]+$/.test(normalized)) return null;
...
function integer(value: string, minimum: number, maximum: number, path: string, errors: Errors): number | null {
  if (!/^(?:0|[1-9][0-9]*)$/.test(value)) { error(errors, "invalid_value", path, "Expected a base-10 integer"); return null; }
  const parsed = Number(value);
  if (!Number.isSafeInteger(parsed) || parsed < minimum || parsed > maximum) { error(errors, "invalid_value", path, `Value must be between ${minimum} and ${maximum}`); return null; }
  return parsed;
}
function list<T extends string>(
  values: string[], normalize: (value: string) => T | null, path: string,
  minimum: number, maximum: number, errors: Errors, sort = true,
): T[] {
  if (values.length < minimum || values.length > maximum) error(errors, "invalid_value", path, `List must contain between ${minimum} and ${maximum} items`);
  const output: T[] = [];
  const seen = new Set<string>();
  values.forEach((value, index) => {
    const normalized = normalize(value);
    if (!normalized) error(errors, "invalid_value", `${path}[${index}]`, "Invalid or unsafe value");
    else if (seen.has(normalized)) error(errors, "duplicate_value", `${path}[${index}]`, "Duplicate value after normalisation");
    else { seen.add(normalized); output.push(normalized); }
  });
  return sort ? output.sort((left, right) => left.localeCompare(right)) : output;
}
...
function assignList(result: RawFrontMatter, key: string, values: string[]): void {
  if (key === "repositories" || key === "runner_profiles" || key === "autonomous_actions" || key === "approval_required" || key === "checks") result[key] = values;
}
...
function setChange(previous: readonly string[], proposed: readonly string[], trackOrder = false): ProjectAttachmentSetChange {
  ...
  return {
    ...
    orderChanged: trackOrder && previous.length === proposed.length && previous.some((value, index) => value !== proposed[index]),
  };
}
function numberChange(previous: number, proposed: number): "increased" | "decreased" | "unchanged" {
  return proposed > previous ? "increased" : proposed < previous ? "decreased" : "unchanged";
}
function changed(change: ProjectAttachmentSetChange): boolean { return change.added.length > 0 || change.removed.length > 0; }
function error(errors: Errors, code: ProjectAttachmentValidationError["code"], path: string, message: string): void { errors.push({ code, path, message }); }
function failed(errors: Errors): ProjectAttachmentParseResult {
  return { ok: false, errors: [...errors].sort((a, b) => a.path.localeCompare(b.path) || a.code.localeCompare(b.code)) };
}
