...
export function parseResourceSettlementReceipt(input: unknown): ResourceSettlementReceipt {
  ...
  if (JSON.stringify(record.counts) !== JSON.stringify(reconstructed.counts)) {
    throw new Error("Resource settlement counts are not derived correctly");
  }
  if (JSON.stringify(record.successfulOutputs) !== JSON.stringify(reconstructed.successfulOutputs)) {
    throw new Error("Resource settlement outputs are not derived correctly");
  }
  if (JSON.stringify(record.reconciliationOwnerKeys) !== JSON.stringify(reconstructed.reconciliationOwnerKeys)) {
...
function parseOwners(
  ...
  if (!Array.isArray(input) || input.length > 500) {
    throw new Error("Resource settlement owners must be an array with at most 500 entries");
  }
  const owners = input.map((entry) => parseOwner(entry, openedAt, closingStartedAt, observedAt));
  const sorted = [...owners].sort((left, right) => ownerKey(left).localeCompare(ownerKey(right)));
...
function parseReceiptOwners(input: unknown): ResourceSettlementOwner[] {
  if (!Array.isArray(input) || input.length > 500) throw new Error("Resource settlement receipt owners are invalid");
