...
export class OpenAIAgentsRunnerAdapter implements RunnerAdapterV1 {
  ...
  async requestCheckpoint(
    ...
    if (!checkpoint) {
      throw new RangeError("OpenAI Agents run has no published checkpoint");
    }
    return checkpoint;
  }
  ...
  async #publishCheckpoint(
    ...
    const append: OpenAIAgentsCheckpointAppendV1 = {
      version: 1,
      publicationKey: sha256(stableJson([
        command.commandId,
        command.kind,
        command.runId,
  ...
  async #publishArtifact(
    ...
    const saved = await this.#externalStore.saveArtifact({
      version: 1,
      adapterId: this.#descriptor.adapterId,
      adapterVersion: this.#descriptor.adapterVersion,
      profileId: command.profileId,
...
function admitControlCommandV1(
  ...
  if (input.version !== RUNNER_ADAPTER_V1) {
    throw new RangeError("OpenAI Agents control command version is invalid");
  }
  const runId = boundedSafeText(
    input.runId,
    160,
  ...
  const base = {
    version: RUNNER_ADAPTER_V1,
    commandId: boundedSafeText(
      input.commandId,
      160,
      "OpenAI Agents control command ID",
    ),
    adapterId: boundedSafeText(
      input.adapterId,
      160,
      "OpenAI Agents control adapter ID",
    ),
    adapterVersion: boundedSafeText(
      input.adapterVersion,
      160,
      "OpenAI Agents control adapter version",
    ),
    profileId: boundedSafeText(
      input.profileId,
      160,
...
function admitControlAuthorityV1(
  ...
  const resource = boundedSafeText(
    input.resource,
    320,
...
function admitCancellationObservationV1(
  ...
  return Object.freeze({
    version: RUNNER_ADAPTER_V1,
    commandId: boundedSafeText(input.commandId, 160, "Cancellation command ID"),
    adapterId: boundedSafeText(input.adapterId, 160, "Cancellation adapter ID"),
    adapterVersion: boundedSafeText(
      input.adapterVersion,
      160,
      "Cancellation adapter version",
    ),
    profileId: boundedSafeText(input.profileId, 160, "Cancellation profile ID"),
    runId: boundedSafeText(input.runId, 160, "Cancellation run ID"),
    runGeneration: checkpointInteger(input.runGeneration, 0),
    leaseGeneration: checkpointInteger(input.leaseGeneration, 1),
