...
export class OpenAIAgentsRunnerAdapter extends BaseOpenAIAgentsRunnerAdapter {
  ...
  readonly #pendingAuthorityHolders = new Map<string, string>();
  ...
  override async *start(
    ...
    const reservation = this.#reserveAuthority(command);
    let accepted = false;
    try {
      ...
      this.#releaseAuthorityReservation(reservation, accepted);
    }
  }
  ...
  #reserveAuthority(
    ...
    const holderId = command.authority.holderId;
    const admittedHolder = this.#authorityHolders.get(key);
    if (admittedHolder === holderId) {
      return { key, holderId, created: false };
    }
    if (admittedHolder !== undefined || this.#latestCheckpoints.has(key)) {
      throw new RangeError(
        "OpenAI Agents control command authority holder is stale",
      );
    }
    const pendingHolder = this.#pendingAuthorityHolders.get(key);
    if (pendingHolder !== undefined) {
      if (pendingHolder !== holderId) {
        throw new RangeError(
          "OpenAI Agents control command authority holder is stale",
        );
      }
      throw new RangeError(
        "OpenAI Agents authority admission is already in progress",
      );
    }
    if (this.#pendingAuthorityHolders.size >= maximumControlEntries) {
      throw new RangeError(
        "OpenAI Agents authority admission capacity is exhausted",
      );
    }
    this.#pendingAuthorityHolders.set(key, holderId);
    return { key, holderId, created: true };
  }
  ...
  #releaseAuthorityReservation(
    ...
    if (!reservation.created || accepted) return;
    if (
      this.#pendingAuthorityHolders.get(reservation.key)
        === reservation.holderId
    ) {
      this.#pendingAuthorityHolders.delete(reservation.key);
    }
  }
  ...
  #admitAuthority(command: RunnerStartCommandV1 | RunnerResumeCommandV1): void {
    ...
    if (
      this.#pendingAuthorityHolders.get(key) === command.authority.holderId
    ) {
      this.#pendingAuthorityHolders.delete(key);
    }
  }
