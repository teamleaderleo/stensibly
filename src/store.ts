...
export class StensiblyStore {
  ...
  private migrate(): void {
    ...
    const itemColumns = this.db
      ...
      .all();
    if (!itemColumns.some((column) => column.name === "claim_generation")) {
      this.db.exec(
  ...
  releaseItem(
    ...
    const transaction = this.db.transaction(() => {
      ...
      this.getItem(id);
      const generation = claimGeneration(expectedClaimGeneration);
      const now = new Date().toISOString();
      ...
      const result = this.db
        ...
        `)
        .run(now, id, actor.id, generation);
      ...
      const result = this.db
        .query(`
          ...
          SET status = 'done',
              ...
              claimed_by = NULL,
              claim_expires_at = NULL,
              claim_generation = claim_generation + CASE
                WHEN claimed_by IS NOT NULL OR claim_expires_at IS NOT NULL THEN 1
                ELSE 0
              END,
              version = version + 1,
              updated_at = ?2
