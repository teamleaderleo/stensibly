...
describe("dashboard board filter integration", () => {
  ...
  test("reapplies after polling without an observer loop", () => {
    ...
    expect(controller).toContain("if (baseline.textContent !== baselineText) baseline.textContent = baselineText");
  });
  ...
  test("updates visible counts, status columns, and honest empty states", () => {
    ...
    expect(controller).toContain("No items exist in this status.");
  ...
  test("keeps types and responsive presentation aligned", () => {
    ...
    expect(styles).toContain("background: rgba(21, 21, 24, 0.9)");
    expect(styles).not.toContain("var(--paper)");
