...
describe("effective tool-surface host-binding caller admission", () => {
  ...
  test("normalizes revoked top-level, tool-surface, class, and capability inputs", () => {
    ...
    const classes = withClass.toolSurface.classes as unknown as Record<string, unknown>;
    const classTarget = classes.native_core as object;
    const classValue = Proxy.revocable(classTarget, {});
    classValue.revoke();
    classes.native_core = classValue.proxy;
    expect(() => buildEffectiveToolSurfaceHostBindingSnapshot(withClass))
      .toThrow("Effective tool-surface host-binding input inspection failed");

    const withCapability = input();
    const capabilityClasses = withCapability.toolSurface.classes as unknown as Record<
      string,
      ToolSurfaceClassInput
    >;
    const native = capabilityClasses.native_core!;
    const capability = Proxy.revocable(native.executable[0]!, {});
    capability.revoke();
    (native as unknown as { executable: unknown[] }).executable = [capability.proxy];
  ...
  test("rejects accessor-bearing nested fields without invoking getters", () => {
    let getterCalls = 0;
    const value = input();
    const classes = value.toolSurface.classes as unknown as Record<string, ToolSurfaceClassInput>;
    const native = classes.native_core! as unknown as Record<string, unknown>;
  ...
  test("checks capability array length before entry inspection", () => {
    ...
    const classes = value.toolSurface.classes as unknown as Record<string, ToolSurfaceClassInput>;
    const native = classes.native_core!;
    (native as unknown as { executable: unknown[] }).executable = proxied;
