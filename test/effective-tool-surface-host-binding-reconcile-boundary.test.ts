...
describe("effective tool-surface host-binding reconciliation boundary", () => {
  ...
  test("rejects wrapper snapshots without executing get or ownKeys traps", () => {
    const previous = buildEffectiveToolSurfaceHostBindingSnapshot(input("previous"));
    const current = buildEffectiveToolSurfaceHostBindingSnapshot(input("current"));
    let getCalls = 0;
    let ownKeysCalls = 0;
  ...
  test("rejects a structured clone that did not come from this compiler instance", () => {
    const genuine = buildEffectiveToolSurfaceHostBindingSnapshot(input("current"));
    const clone = structuredClone(genuine) as EffectiveToolSurfaceHostBindingSnapshot;

    expect(() => reconcileEffectiveToolSurfaceHostBinding(clone)).toThrow(
      "Effective tool-surface host-binding snapshot inspection failed",
    );
  });
});
