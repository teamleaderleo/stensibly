...
describe("frontend labs deterministic evidence plan", () => {
  ...
  test("emits only manifest-supported profiles, presentation axes, and state scenarios", () => {
    ...
    for (const variant of frontendLabManifest) {
      const cases = plan.cases.filter((entry) => entry.variantId === variant.id);
      for (const profile of frontendLabEvidenceProfiles) {
        expect(cases.some((entry) => entry.profileId === profile.id)).toBe(
          variant.support.includes(profile.requiredSupport),
        );
      }
      for (const colorScheme of ["light", "dark"] as const) {
        expect(cases.some((entry) => entry.colorScheme === colorScheme)).toBe(
          variant.support.includes(colorScheme),
        );
      }
