...
describe("Quiet Control shared fixture and keyboard contract", () => {
  ...
  test("supports both initialization orders and rejects mutable, accessor, or incompatible globals", () => {
    ...
    expect(moduleFixture).toContain("Object.getOwnPropertyDescriptors");
    expect(moduleFixture).toContain("apiFromDescriptor");
    expect(moduleFixture).toContain("incompatible contract");
    expect(classicFixture).toContain("Object.getOwnPropertyDescriptors");
    ...
    runIsolatedFixtureCheck(`
      ...
      Object.defineProperty(fakeApi, "frontendLabFixture", {
        get() {
          getterCalls += 1;
          return Object.freeze({});
        },
      ...
      let rejected = false;
      try {
        await import(${JSON.stringify(`${moduleFixtureUrl}?api-field-accessor`)});
      } catch (error) {
        rejected = String(error).includes("incompatible contract");
      }
      if (!rejected) throw new Error("fixture API field accessor was accepted");
      if (getterCalls !== 0) throw new Error("fixture API field accessor was invoked");
    `);
    ...
    runIsolatedFixtureCheck(`
      ...
      Object.defineProperty(fakeApi, "frontendLabTasks", {
        get() {
          getterCalls += 1;
          return Object.freeze([]);
        },
      ...
      if (!rejected) throw new Error("classic fixture API field accessor was accepted");
      if (getterCalls !== 0) throw new Error("classic fixture API field accessor was invoked");
    `);
  });
  ...
  test("restores keyboard-activated nav focus after the app replaces the nav tree", () => {
    ...
    const dispatchClick = clickListener as unknown as (event: { detail: number; target: FakeElement }) => void;

    const oldButton = new FakeElement("active");
    buttons = [oldButton];
    dispatchClick({ detail: 0, target: oldButton });
    ...
    dispatchClick({ detail: 1, target: replacementButton });
