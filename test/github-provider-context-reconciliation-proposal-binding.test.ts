...
describe("GitHub provider context reconciliation proposal binding", () => {
  ...
  test("rejects provider revisions unsafe for context acceptance", () => {
    const unsafeResult = buildGitHubIssueContext({
      owner: "teamleaderleo",
      repository: "stensibly",
      number: 958,
      title: "Compile verified GitHub issue receipts into context reconciliation proposals",
    ...
    expect(() => compile(receipt({
      ...
      verification: {
        state: "passed",
        checkedAt: "2026-08-02T17:27:00.000Z",
        sourceRevision: unsafeResult.sourceRevision,
      },
    }))).toThrow("GitHub issue source revision cannot be credential-shaped");
  });
