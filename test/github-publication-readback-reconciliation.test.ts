...
describe("GitHub publication readback reconciliation", () => {
  test("settles an ambiguous branch creation from the exact canonical branch without redispatch", async () => {
    ...
    const input = branchInput("readback-branch-exact");
  ...
  test("keeps absent and moved branch observations pending", async () => {
    for (const mode of ["absent", "moved"] as const) {
      ...
      const receipts = new InMemoryGitHubProviderReceiptStore();
      const { publication, reconciliation } = services(adapter, receipts);
      const input = branchInput(`readback-branch-${mode}`);
  ...
  test("settles a PR whose mutation identity survived a lost verification read", async () => {
    const adapter = new FakePublicationAdapter();
    adapter.branches.set("kite/readback-pr", branchResult("kite/readback-pr", headSha));
    adapter.failPullRequestReadback = true;
    const receipts = new InMemoryGitHubProviderReceiptStore();
    const { publication, reconciliation } = services(adapter, receipts);
    const input = pullRequestInput("readback-pr-exact");
  ...
  test("keeps changed PR state pending and preserves the original receipt", async () => {
    const adapter = new FakePublicationAdapter();
    adapter.branches.set("kite/readback-pr", branchResult("kite/readback-pr", headSha));
    adapter.failPullRequestReadback = true;
    const receipts = new InMemoryGitHubProviderReceiptStore();
    const { publication, reconciliation } = services(adapter, receipts);
    const input = pullRequestInput("readback-pr-mismatch");

    await expect(publication.createPullRequest(input)).rejects.toMatchObject({
      name: "GitHubProviderPendingReconciliationError",
    });
    const original = await receipts.getGitHubProviderReceipt(
      project,
      input.idempotencyKey,
    );
    adapter.failPullRequestReadback = false;
    const retained = adapter.pullRequests.get(42)!;
    adapter.pullRequests.set(42, pullRequestResult({
    ...
    expect(await reconciliation.reconcilePullRequest(input)).toEqual(original);
    expect(adapter.pullRequestWrites).toBe(1);
  });

  test("keeps a PR with no retained provider identity pending without guessing", async () => {
    const adapter = new FakePublicationAdapter();
    adapter.branches.set("kite/readback-pr", branchResult("kite/readback-pr", headSha));
    adapter.ambiguousPullRequestWithoutResult = true;
    const receipts = new InMemoryGitHubProviderReceiptStore();
    const { publication, reconciliation } = services(adapter, receipts);
    const input = pullRequestInput("readback-pr-no-identity");

    await expect(publication.createPullRequest(input)).rejects.toMatchObject({
      name: "GitHubProviderPendingReconciliationError",
      receipt: { state: "pending_reconciliation", result: null },
    });
    const readsBefore = adapter.pullRequestReads;
    const pending = await reconciliation.reconcilePullRequest(input);
    expect(pending.state).toBe("pending_reconciliation");
    expect(adapter.pullRequestReads).toBe(readsBefore);
    expect(adapter.pullRequestWrites).toBe(1);
  });

  test("rejects changed request identity before provider read or settlement", async () => {
    const adapter = new FakePublicationAdapter();
    adapter.ambiguousBranchAfterEffect = true;
    const receipts = new InMemoryGitHubProviderReceiptStore();
    const { publication, reconciliation } = services(adapter, receipts);
    const input = branchInput("readback-identity");
  ...
  test("requires current authority before observing provider state", async () => {
    const adapter = new FakePublicationAdapter();
    adapter.ambiguousBranchAfterEffect = true;
    const receipts = new InMemoryGitHubProviderReceiptStore();
    const live = sharedDependencies(adapter, receipts);
    const publication = new GitHubPublicationProviderService(live);
    const input = branchInput("readback-authority");

    await expect(publication.createBranch(input)).rejects.toMatchObject({
      name: "GitHubProviderPendingReconciliationError",
    });
    const readsBefore = adapter.branchReads;
    const reconciliation = new GitHubPublicationReadbackReconciler({
      ...live,
      authority: {
    ...
    await expect(reconciliation.reconcileBranch(input)).rejects.toThrow(
