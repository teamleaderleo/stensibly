...
describe("pull request reconciliation shared retained credential policy", () => {
  test("rejects realistic Stensibly provider identities at the shared 12-character threshold", () => {
    ...
    expect(() => compile(providerRead({ bindingId: serviceIdentity })))
      .toThrow("GitHub delegated binding ID is invalid");
    expect(() => compile(providerRead({ providerRequestId: tokenIdentity })))
      .toThrow("GitHub provider request ID is invalid");
    expect(() => compile(providerRead({ actorId: serviceIdentity })))
      .toThrow("GitHub delegated actor ID is invalid");
    expect(() => compile(providerRead({ clientId: tokenIdentity })))
      .toThrow("GitHub delegated client ID is invalid");
  });

  test("screens other receipt metadata identities without changing provider content admission", () => {
    const serviceIdentity = `stn.svc_${"c".repeat(12)}`;

    for (const overrides of [
      { connectionId: serviceIdentity },
      { installationId: serviceIdentity },
      { attachmentId: serviceIdentity },
    ] satisfies Partial<GitHubDelegatedReadReceipt>[]) {
      expect(() => compile(providerRead(overrides))).toThrow();
    }
  });
  ...
  test("retains benign Stensibly-like aliases below the shared threshold", () => {
    ...
    expect(result.reconciliationFingerprint).toMatch(/^sha256:[a-f0-9]{64}$/u);
  });
});
