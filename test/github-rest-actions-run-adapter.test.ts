...
describe("native GitHub delegated Actions run and job reads", () => {
  ...
  test("reads the latest jobs for one exact run and verifies repository identity from each job URL", async () => {
    const tokens = new RecordingTokenProvider();
    let requestUrl = "";
    ...
    expect(requestUrl).toBe(
      `https://api.github.test/repos/teamleaderleo/stensibly/actions/runs/${runId}/jobs?filter=latest&per_page=100&page=1`,
    );
  ...
  test("rejects caller and provider identity drift before publication", async () => {
    ...
    await expectRejectionCode(
      adapter.callReadTool(callInput("fetch_workflow_run_jobs", { run_id: "30660326044" })),
    ...
    const malformedSha = createAdapter(new RecordingTokenProvider(), async () =>
      Response.json({
        total_count: 1,
        workflow_runs: [{ ...workflowRun(), head_sha: "malformed" }],
      }));
    await expectRejectionCode(
      malformedSha.callReadTool(callInput(
        "fetch_commit_workflow_runs",
        { commit_sha: commitSha },
      )),
  ...
  test("rejects pagination escape, inconsistent counts, run mismatch, and mixed job identity", async () => {
    ...
    const attempts = createAdapter(new RecordingTokenProvider(), async () =>
      Response.json({
        total_count: 2,
        jobs: [
          workflowJob(),
          {
            ...
            id: 778,
            run_attempt: 2,
            url: "https://api.github.test/repos/teamleaderleo/stensibly/actions/jobs/778",
          },
        ],
      }));
    await expect(attempts.callReadTool(callInput(
      "fetch_workflow_run_jobs",
      { run_id: runId },
    ))).rejects.toThrow("one run attempt and head commit");

    const heads = createAdapter(new RecordingTokenProvider(), async () =>
      Response.json({
        total_count: 2,
        jobs: [
          workflowJob(),
    ...
    await expect(heads.callReadTool(callInput(
      "fetch_workflow_run_jobs",
      { run_id: runId },
    ))).rejects.toThrow("one run attempt and head commit");
  });
