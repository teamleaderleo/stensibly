...
describe("delegated provider URL and receipt identity", () => {
  ...
  test("rejects direct and delimiter-namespaced credential-shaped provider request IDs", async () => {
    const content = Buffer.from("privacy-bound content", "utf8");
    for (const providerRequestId of [
      "github_pat_example",
      "trace/github_pat_example",
      "ghp_example",
      "receipt:stn.tok_example",
      "audit/sk-proj-example",
      "xoxb-example",
      "env://GITHUB_TOKEN",
      ...
      const adapter = createAdapter(async () => Response.json(filePayload(
        ...
        headers: { "x-github-request-id": providerRequestId },
      }));
