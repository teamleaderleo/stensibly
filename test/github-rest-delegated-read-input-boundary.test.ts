...
test("reads declared call fields without caller key enumeration", async () => {
  ...
  const rejectedCall = instance.callReadTool(
    proxied as ReturnType<typeof validCall>,
  );
  await expect(rejectedCall)
    .rejects.toThrow("request failed before a response was available");
  await expect(rejectedCall)
    .rejects.not.toThrow("provider dispatch must remain private");
