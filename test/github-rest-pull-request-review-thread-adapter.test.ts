...
describe("native GitHub pull request review-thread reads", () => {
  ...
  test("retains resolved state when resolver attribution is unavailable", async () => {
    const tokenProvider = new RecordingTokenProvider();
    const adapter = createAdapter(tokenProvider, async () => graphqlResponse(
      pagePayload([{ ...threadOne(), resolvedBy: null }], 1, false, null),
      "LIFECYCLE:RESOLVER",
    ));
    ...
    expect(called.result).toMatchObject({
      threadCount: 1,
      threads: [{ resolved: true, resolvedByLogin: null }],
    });
  });

  test("binds pending and submitted review states to submission timestamps", async () => {
    const submittedWithoutTime = {
      ...threadOne(),
      comments: commentsConnection([
        comment("PRRC_submitted_without_time", "Missing submission time.", {
          review: {
            id: "PRR_submitted_without_time",
            state: "COMMENTED",
            submittedAt: null,
    ...
    const rejectedAdapter = createAdapter(
      ...
      async () => graphqlResponse(
        pagePayload([submittedWithoutTime], 1, false, null),
        "LIFECYCLE:SUBMITTED",
      ),
    );
    await expect(rejectedAdapter.callReadTool(callInput(
      "list_pull_request_review_threads",
      { pr_number: pullRequestNumber },
    ))).rejects.toThrow("review state and submission timestamp were inconsistent");

    const pendingWithoutTime = {
      ...threadOne(),
      comments: commentsConnection([
        comment("PRRC_pending_without_time", "Pending review.", {
          review: {
            id: "PRR_pending_without_time",
            state: "PENDING",
            submittedAt: null,
            author: { login: "author-one" },
          },
    ...
    const acceptedAdapter = createAdapter(
      new RecordingTokenProvider(),
      async () => graphqlResponse(
        pagePayload([pendingWithoutTime], 1, false, null),
        "LIFECYCLE:PENDING",
      ),
    );
    const called = await acceptedAdapter.callReadTool(callInput(
      "list_pull_request_review_threads",
      { pr_number: pullRequestNumber },
    ));
    expect(called.result).toMatchObject({
      threads: [{ comments: [{ review: { state: "PENDING", submittedAt: null } }] }],
    });
  });
