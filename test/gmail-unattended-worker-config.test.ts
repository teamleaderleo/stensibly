...
test("normalizes surrounding staging whitespace on the protected subscription resource", () => {
  const mount = createGmailUnattendedMountFromEnv({
    CONVEX_URL: "https://example.convex.cloud",
    STENSIBLY_SERVICE_SECRET: "service-secret",
    STENSIBLY_GMAIL_OAUTH_CLIENT_ID: "client-id",
    STENSIBLY_GMAIL_OAUTH_CLIENT_SECRET: "client-secret",
    ...
    STENSIBLY_GMAIL_PUBSUB_SUBSCRIPTION:
      "  projects/example/subscriptions/stensibly-gmail-handoffs\n",
    STENSIBLY_GMAIL_PUBSUB_AUDIENCE: "https://api.stensibly.com/internal/gmail/pubsub",
    STENSIBLY_GMAIL_PUBSUB_SERVICE_ACCOUNT:
      "stensibly-gmail-push@example.iam.gserviceaccount.com",
