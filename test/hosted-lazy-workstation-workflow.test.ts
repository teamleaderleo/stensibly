...
test("hosted Lazy verification workflow is exact, bounded, and secret-safe", () => {
  ...
  expect(workflow).toContain("operation_revision:");
  expect(workflow).toContain("HOSTED_LAZY_RUN_REF: ${{ inputs.run_ref || github.run_id }}");
  expect(workflow).toContain("HOSTED_LAZY_OPERATION_REVISION: ${{ inputs.operation_revision || inputs.revision }}");
  expect(workflow).toContain(".operationRevision == $expectedOperationRevision");
