  ...
  if (scenario === "discovery-error") {
    ...
    response = apexCalls === 1
      ...
      : apexCalls === 2
        ? page([], 20)
        : page([], 10);
  } else if (scenario === "cursor-repeat") {
    status = 200;
    response = page([], 10);
  } else if (scenario === "invalid-cursor") {
    status = 200;
    response = page([], "10&limit=10000");
  } else if (scenario === "zero-cursor") {
    status = 200;
    response = page([], 0);
  } else if (scenario === "invalid-source") {
    status = 200;
    response = page([{ name: "www.stensibly.com", projectId: "../../other" }]);
  } else if (scenario === "malformed-record") {
    status = 200;
    response = page([{ name: "www.stensibly.com", projectId: null }]);
  } else {
    status = 200;
...
function apexCalls(result: ScenarioResult): readonly CurlCall[] {
  return result.calls.filter((call) =>
    call.url.includes("/v1/domains/stensibly.com/project-domains?"),
  );
}
...
describe("Vercel canonical project-domain linker", () => {
  ...
  test("rejects a non-adjacent cursor cycle before another provider read", () => {
    const result = runScenario("cursor-cycle");

    expect(result.status).toBe(1);
    expect(apexCalls(result)).toHaveLength(3);
    expect(postCalls(result)).toHaveLength(0);
  ...
  test("rejects an immediately repeated cursor before another provider read", () => {
    const result = runScenario("cursor-repeat");

    expect(result.status).toBe(1);
    expect(apexCalls(result)).toHaveLength(2);
    expect(postCalls(result)).toHaveLength(0);
    expect(result.stdout).toContain(
      "Vercel repeated the project-domain pagination cursor.",
    );
  ...
  test("rejects malformed and zero cursors before URL reuse", () => {
    for (const scenario of ["invalid-cursor", "zero-cursor"]) {
      const result = runScenario(scenario);

      expect(result.status).toBe(1);
      expect(apexCalls(result)).toHaveLength(1);
      expect(postCalls(result)).toHaveLength(0);
      expect(result.stdout).toContain(
  ...
  test("fails closed on malformed discovery records without mutation", () => {
    const result = runScenario("malformed-record");
