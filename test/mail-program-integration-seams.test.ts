...
const canonicalHandle = createMailThreadHandle("handoff", "K8R4");
...
describe("P0 email programme integration seams", () => {
  test("uses one eye-safe STN handle grammar across outbound, UX, and GitHub bridge", () => {
    expect(canonicalHandle).toBe("STN-HANDOFF:K8R4");
    expect(renderMaterialMailMessage(uxThread(canonicalHandle)).launchLine)
      .toBe("Continue STN-HANDOFF:K8R4.");

    for (const invalid of ["STN-HANDOFF:O0O0", "STN-HANDOFF:ABCDEFGH2"]) {
      expect(() => parseMailThreadHandle(invalid)).toThrow("Mail thread handle is invalid");
      expect(() => classifyGitHubMailReply(bridgeReply(invalid)))
