...
describe("guarded GitHub combined-status delegated-read MCP dispatch", () => {
  test("advertises and dispatches the exact commit status read", async () => {
    ...
    const client = new Client(
      { name: "github-commit-status-delegated-mounted", version: "0.0.1" },
      { capabilities: {} },
    );
    ...
    try {
      ...
      expect(toolsets.delegatedTools).toContain("get_commit_combined_status");
      ...
      expect(receipt).toMatchObject({
        ...
        tool: "get_commit_combined_status",
        actorId: "api-token:delegated-status-token",
        clientId: "mcp:api-token:delegated-status-token",
        result: {
          repositoryFullName: repository,
          commitSha,
          state: "success",
  ...
  const provider: HostedGitHubDelegatedReadProvider = {
    ...
    async callGitHubDelegatedRead(input) {
      ...
      return Object.freeze({
        ...
        connectionId: "ghconn_status_dispatch",
        installationId: "98765",
        bindingId: "ghbind_status_dispatch",
        attachmentId: "attach_status_dispatch",
        ...
        providerRequestId: "STATUS:PUBLIC:1",
        resultSha256: `sha256:${"d".repeat(64)}`,
        result: Object.freeze({
          repositoryFullName: input.repository.toLowerCase(),
          commitSha: input.arguments.commit_sha,
          state: "success",
          totalCount: 0,
...
function readPrincipal(): TokenPrincipal {
  return {
    tokenId: "delegated-status-token",
    name: "delegated status test",
    scopes: ["read"],
    projects: [project],
  };
}
