...
describe("hosted MCP first-read evidence path", () => {
  ...
  test("registers a pending first-read write with request lifetime without delaying the successful read", async () => {
    ...
    const responsePromise = call({
      ...
      mcpSetupFirstReadRecorder: {
        async recordSetupFirstRead(input: { accountId: string; project: string }) {
          expect(input).toEqual({ accountId: "acct_test", project: "scrapbook" });
          recorderStarted();
          await blocked;
        },
      },
