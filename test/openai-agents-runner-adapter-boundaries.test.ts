...
class BoundaryFactory implements OpenAIAgentsRuntimeFactory {
  ...
  create(input: {
    ...
    this.createCalls += 1;
    const response = this.completeImmediately
      ? finalResponse()
      : approvalResponse(
...
class BoundaryStore implements OpenAIAgentsExternalStore {
  ...
  artifactReceiptOverride: unknown | undefined;
  ...
  async saveArtifact(
    ...
    if (this.artifactReceiptOverride !== undefined) {
      return this.artifactReceiptOverride as { externalId: string };
    }
...
describe("OpenAI Agents adapter hostile boundaries", () => {
  ...
  test("rejects accessor-bearing artifact receipts after one bounded write", async () => {
    const store = new BoundaryStore();
    let getterReads = 0;
    const hostile: Record<string, unknown> = { externalId: "artifact:hostile" };
    ...
    store.artifactReceiptOverride = hostile;
    const factory = new BoundaryFactory();
    factory.completeImmediately = true;
    const candidate = createAdapter(store, factory);
    await candidate.adapter.inspectCapabilities(startProbe());
    const observations = await collect(candidate.adapter.start(startCommand()));

    expect(observations.at(-1)?.type).toBe("failure_observed");
    expect(observations.some((entry) => entry.type === "artifact_published"))
      .toBe(false);
    expect(JSON.stringify(observations)).not.toContain(secret);
    expect(store.artifactWrites).toBe(1);
    expect(getterReads).toBe(0);
  });
  ...
  test("rejected checkpoints leave recovery state untouched", async () => {
    ...
    ).rejects.toThrow("unknown to this adapter instance");
