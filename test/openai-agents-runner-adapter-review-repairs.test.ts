...
describe("OpenAI Agents adapter review repairs", () => {
  ...
  test("validates the complete completion before any artifact effect", async () => {
    ...
    const invalidCompletions: unknown[] = [
      ...
      { outcome: "completed", executionActual: { toolCalls: secret } },
  ...
  test("admits cancellation once and rejects competing, expired, malformed, and accessor input", async () => {
    const store = new MemoryStore();
    let calls = 0;
    ...
    await expect(adapter.requestCancellation({
      ...
      authority: {
        ...
        holderId: "competing-review-actor",
      },
    ...
    await expect((adapter.requestCancellation as any)({
      ...cancellationCommand(),
      runId: " run_openai_agents_review_repairs ",
    })).rejects.toThrow("authority binding is invalid");
