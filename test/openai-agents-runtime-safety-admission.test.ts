...
describe("OpenAI Agents runtime manifest admission", () => {
  test("re-admits an equivalent plain manifest and returns a frozen copy", () => {
  ...
  test("rejects unknown fields and forged nested records", () => {
    const accepted = manifest();
    const unknown = structuredClone(accepted) as Record<string, unknown>;
  ...
  test("rejects sparse and decorated manifest arrays", () => {
    const accepted = manifest();
