...
describe("orchestrator activity observation inspection", () => {
  ...
  test("inspects only closed fields without caller own-key enumeration", () => {
    let ownKeysCalls = 0;
    const observed = new Proxy(
      {
        ...input(),
        prompt: "discarded private text",
      },
      {
        ownKeys() {
          ownKeysCalls += 1;
          throw new Error("caller keys must not be enumerated");
        },
    ...
    const observation = compileOrchestratorActivityObservation(observed);
    expect(observation.sourceId).toBe("event_1149");
    expect(JSON.stringify(observation)).not.toContain("discarded private text");
