...
describe("orchestrator causal attention projection", () => {
  ...
  test("ignores caller decorations without key enumeration and rejects custom prototypes", () => {
    const base = ambiguousReceipt();
    const decorated = [base] as OrchestratorActivityObservation[] & {
      note?: string;
    };
    ...
    expect(compileOrchestratorAttentionProjection(hostile).observationCount).toBe(1);
    expect(outerOwnKeysCalls).toBe(0);

    let observationOwnKeysCalls = 0;
    const hostileObservation = new Proxy(
      { ...base, decoration: "discard" },
      {
        ownKeys() {
    ...
    let attentionOwnKeysCalls = 0;
    const hostileAttention = new Proxy({ ...base.attention }, {
      ownKeys() {
        attentionOwnKeysCalls += 1;
        throw new Error("attention keys must remain unreachable");
      },
    });
    expect(compileOrchestratorAttentionProjection([{ ...base, attention: hostileAttention }]).observationCount)
      .toBe(1);
    expect(attentionOwnKeysCalls).toBe(0);

    let disclosureOwnKeysCalls = 0;
    const hostileDisclosure = new Proxy({ ...base.disclosure }, {
      ownKeys() {
        disclosureOwnKeysCalls += 1;
        throw new Error("disclosure keys must remain unreachable");
      },
    });
    expect(compileOrchestratorAttentionProjection([{ ...base, disclosure: hostileDisclosure }]).observationCount)
      .toBe(1);
    expect(disclosureOwnKeysCalls).toBe(0);

    let evidenceOwnKeysCalls = 0;
    const hostileEvidence = new Proxy([...base.relatedEvidenceIds], {
      ownKeys() {
        evidenceOwnKeysCalls += 1;
        throw new Error("evidence keys must remain unreachable");
      },
    });
    expect(compileOrchestratorAttentionProjection([{ ...base, relatedEvidenceIds: hostileEvidence }]).observationCount)
      .toBe(1);
    expect(evidenceOwnKeysCalls).toBe(0);
