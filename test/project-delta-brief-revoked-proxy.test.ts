...
describe("project delta revoked-proxy inspection", () => {
  ...
  test("normalizes a revoked input array to the fixed input diagnostic", () => {
    const value = input();
    const { proxy, revoke } = Proxy.revocable(value.observations, {});
    value.observations = proxy;
    revoke();

    expect(() => compileProjectDeltaBrief(value))
      .toThrow("Project delta input inspection failed");
  });
  ...
  test("normalizes a revoked render array to the fixed brief diagnostic", () => {
    const brief = compileProjectDeltaBrief(input());
    const { proxy, revoke } = Proxy.revocable(brief.completed, {});
    const forged = { ...brief, completed: proxy } as ProjectDeltaBrief;
    revoke();

    expect(() => renderProjectDeltaBriefMarkdown(forged))
      .toThrow("Project delta brief inspection failed");
  });
});
