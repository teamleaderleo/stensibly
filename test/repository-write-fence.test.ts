...
describe("repository write fence", () => {
  test("rejects an unknown branch_name field before provider dispatch", () => {
    let providerDispatches = 0;
    expect(() => {
      const prepared = prepareRepositoryWrite(intent({ branch_name: "wrong-branch" }));
  ...
  test("allows no second write until the first returned commit is verified", async () => {
    let expectedParentSha = parent;
    let verificationPending = false;
    let dispatches = 0;

    const dispatch = async (nextCommit: string) => {
      if (verificationPending) throw new Error("prior repository write still requires verification");
      const prepared = prepareRepositoryWrite(intent({ expectedParentSha }));
      verificationPending = true;
      dispatches += 1;
      return { prepared, providerResult: { commitSha: nextCommit } };
    };

    const first = await dispatch(commit);
    await expect(dispatch("c".repeat(40))).rejects.toThrow(/requires verification/);
    ...
    expectedParentSha = verified.nextExpectedParentSha;
    verificationPending = false;

    await dispatch("c".repeat(40));
