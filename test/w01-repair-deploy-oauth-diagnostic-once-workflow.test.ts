...
const workflow = readFileSync(
  new URL(
    "../.github/workflows/w01-repair-deploy-oauth-diagnostic-once.yml",
    import.meta.url,
  ),
  "utf8",
);
...
describe("W01 repaired one-time OAuth callback diagnostic deployment", () => {
  test("activates only when this generation is first added to main", () => {
    ...
    expect(workflow).toContain(
      "ACTIVATION_BEFORE_SHA: ${{ github.event.before }}",
    );
    expect(workflow).toContain(
      'git diff --name-status "$ACTIVATION_BEFORE_SHA" "$GITHUB_SHA" -- "$ACTIVATION_WORKFLOW_PATH"',
    );
    expect(workflow).toContain("$'A\\t'");
    expect(workflow).toContain(
      'git cat-file -e "${ACTIVATION_BEFORE_SHA}:${ACTIVATION_WORKFLOW_PATH}"',
    );
    expect(workflow).toContain('if [[ "$GITHUB_RUN_ATTEMPT" != "1" ]]');
    expect(workflow).not.toContain("head_commit.added");
  ...
  test("pins every third-party action to an immutable commit", () => {
    ...
    expect(workflow).toContain(`oven-sh/setup-bun@${setupBunSha}`);
    expect(workflow).toContain(`actions/upload-artifact@${uploadArtifactSha}`);
    expect(workflow).not.toContain("actions/checkout@v");
    expect(workflow).not.toContain("oven-sh/setup-bun@v");
    expect(workflow).not.toContain("actions/upload-artifact@v");
  });
  ...
  test("retains bounded evidence without secret values", () => {
    expect(workflow).toContain(
      'version "w01-oauth-callback-diagnostic-deploy/v2"',
    );
