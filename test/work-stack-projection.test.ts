...
describe("work-stack projection", () => {
  ...
  test("rejects missing attention time, hostile inputs, ambiguous links, and secret shapes", () => {
    ...
    const revokedInput = Proxy.revocable(input([record(3)]), {});
    revokedInput.revoke();
    expect(() => compileWorkStackProjection(revokedInput.proxy))
      .toThrow("work-stack input could not be inspected");

    const revokedRecords = Proxy.revocable([record(4)], {});
    revokedRecords.revoke();
    expect(() => compileWorkStackProjection(input(revokedRecords.proxy)))
      .toThrow("records could not be inspected");
